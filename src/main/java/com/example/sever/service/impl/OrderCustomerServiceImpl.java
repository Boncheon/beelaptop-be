...
import com.example.sever.dto.OrderDTO.OrderCTCustomerRequest;
...
import com.example.sever.dto.OrderDTO.PaymentCustomerRequest;
...
public class OrderCustomerServiceImpl implements OrderCustomerService {
    ...
    public OrderCustomerResponse taoDonHangCustomer(OrderCustomerRequest request) {
        ...
        for (OrderCTCustomerRequest ctRequest : request.getListOrderCT()) {
            ...
            // Lấy danh sách Seri từ LaptopChiTiet (chỉ lấy 1 seri đầu tiên)
            java.util.List<String> seriIdStrings = seriRepo.findSeriIdsByLaptopChiTietId(laptopChiTietId);
        ...
        if (phieuGiamGia != null && soTienGiam.compareTo(BigDecimal.ZERO) > 0) {
            ...
            giamGiaHoaDon.setTienSauGiam(tongTienChuaGiam.subtract(soTienGiam));
        ...
        try {
            ...
            if (gioHangOpt.isPresent()) {
                ...
                for (Map<String, Object> productInfo : productList) {
                    ...
                    Optional<GioHangChiTiet> gioHangChiTietOpt = gioHangChiTietRepo.findByIdGioHangAndIdSpct(gioHang, laptopChiTietId);
